...
```{r Distribution for fuel consumption in city,echo=FALSE,warning=FALSE,message=FALSE}
p1 <- ggplot(data = Automobile_data, mapping = aes(x = `Fuel Consumption (City (L/100 km)`)) +
          geom_histogram(fill = "white",
                         color = rgb(0, 0.5, 1, alpha = 0.5)) +
          geom_area(stat = "bin", fill = rgb(0, 0.5, 1, alpha = 0.5)) +
          labs(x = "Fuel Consumption (City (L/100 km)",
               y = "Count",
               title = 'Distribution for the fuel efficiency in city',
...
```{r Distribution for fuel consumption in highway,echo=FALSE,warning=FALSE,message=FALSE}
p2 <- ggplot(data = Automobile_data, mapping = aes(x = `Fuel Consumption(Hwy (L/100 km))`)) +
          geom_histogram(fill = "white", 
                         color = rgb(0, 0.5, 1, alpha = 0.5)) +
          geom_area(stat = "bin", fill = rgb(0, 0.5, 1, alpha = 0.5)) +
          labs(x = "Fuel Consumption(Hwy (L/100 km))", 
               y = "Count",
               title = 'Distribution for the Fuel efficiency in Highway',
               caption = "Automobile Dataset found from Kaggle") +
          theme_pubclean()
p2
#ggplotly(p2)
...
p3 <-Automobile_data %>% 
  ...
  labs( x = "Car count", 
            y = "Vehicle Class",
            title = 'Distribution for the Vehicle class category',
            caption = "Automobile Dataset found from Kaggle")+
          theme_pubclean()
p3

# p3 <- ggplot(data= Automobile_data,mapping = aes(y = `Vehicle Class`)) +
...
## 2.2 Distribution of Transmission variable
...
p4 <-Automobile_data %>% 
  ...
  labs( x = "Car count", 
            y = "Transmission",
            title = 'Distribution for the Vehicle Transmission category',
            caption = "Automobile Dataset found from Kaggle")+
          theme_pubclean()
p4

# g2.2 <- ggplot(data = Automobile_data, mapping = aes(y = `body-style`)) +
#           geom_bar() + 
#           labs( x = "Car count",
#                 y = "Body style",
#                 title = 'Distribution for the body style categorical variable',
...
```{r}
g3 <- ggplot( data = Automobile_data, mapping = aes(x = `Fuel Consumption (City (L/100 km)`, y = `Vehicle Class`,fill=`Vehicle Class`)) +
       geom_density_ridges()+ theme_ridges() +
       labs(x = "Fuel Consumption (City (L/100 km)", 
            y = "Vehicle Class",
            title = 'Fuel Consumption (City (L/100 km) VS Vehicle Class',
            caption = "Automobile Dataset found from Kaggle")+  theme(legend.position = "none")
g3
```
...
```{r}
g4.1 <- ggplot(data = Automobile_data, mapping = aes(x = `Fuel Consumption (City (L/100 km)`,y =`Fuel Consumption(Hwy (L/100 km))`)) + 
          geom_point(color = rgb(0, 0.5, 1, alpha = 0.5)) +
          labs(x = "Fuel Consumption in City (L/100 km)", 
               y = "Fuel Consumption in Highway (L/100 km)", 
               title = 'Relationship between Fuel Consumption in city VS highway',
               caption = "Automobile Dataset found from Kaggle")+
...
```{r}
g4.2 <- ggplot(data = Automobile_data) +
          ...
          labs(x = "Vehicle Class",
               ...
               caption = "Automobile Dataset found from Kaggle") + theme(legend.position = "right")+           
  coord_flip()
g4.2
#ggplotly()
```
...
```{r}
g5 <- ggplot(data = Automobile_data,mapping = aes(x =`Fuel Consumption (City (L/100 km)` ,y =`Fuel Consumption(Hwy (L/100 km))`)) +
...
We referred the following websites for enhancing our knowledge in this project.
 ...
 -[The R Graph Galerry](https://r-graph-gallery.com/index.html)  
